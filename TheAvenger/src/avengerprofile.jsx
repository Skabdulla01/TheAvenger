...
function avengerprofile(props) {
  ...
  return (
    ...
    <main className='h-screen overflow-hidden w-full pt-4 flex flex-col items-center bg-[linear-gradient(135deg,#0F172A,#1E1B4B)]'>
      <div ref={topanimation} className="text-[4vw] leading-none font-bold">
      ...
      <div className="flex h-screen w-[90%] justify-between max-[430px]:justify-start max-[430px]:items-center" style={{ flexDirection: `${props.flexdirection}` }}>
        <div className="relative flex justify-center items-center w-[40%]"> 
          <div className="h-[45vw] w-full flex justify-center items-start">
            <div ref={zoomcircle} className="w-[27vw] h-[27vw] rounded-full opacity-80" style={{background: `radial-gradient(circle, ${props.TextStroke} 0%, ${props.rgb} 100%)`}}></div>
          </div>
        ...
        <div ref={txtanimation} className="h-[80vh] max-[430px]:h-[30vh] max-[430px]:bg-green-500 w-[55%] flex flex-col justify-center">
          <h3 className='text-[#ffffff] text-bold text-[3.5vw]' style={{ WebkitTextStroke: `2px ${props.TextStroke}` }}>{props.realname}</h3>
          <p className='text-[#ffffff] text-[1.5vw] drop-shadow-xl'>{props.story}</p>
        </div>
      </div>
    </main>
    </>
